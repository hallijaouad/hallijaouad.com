...
test.describe('Home Page', () => {
    ...
    test('should display the description paragraphs', async ({ page }) => {
        ...
        await expect(paragraphs.nth(0)).toHaveText(/Passionné par le développement web/);
        await expect(paragraphs.nth(1)).toHaveText(/solutions robustes, sécurisées et adaptées/);
        await expect(paragraphs.nth(2)).toHaveText(/assurer la réussite des développeurs/);
    });

    test('should have a working "En savoir plus" link', async ({ page }) => {
        const link = await page.locator('[data-test-id="page_home_link_plus"]');
    ...
    test('should display the profile image with correct attributes', async ({ page }) => {
        const profileImage = await page.locator('[data-test-id="page_home_img_profile"]');
